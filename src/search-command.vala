...
class SearchCommand : Command {
    ...
    public override bool run () throws Error {
        ...
        if (args.length < 2) {
            throw new CommandError.INVALID_ARG ("Search criteria missing");
        }

        if (args.length > 2) {
            throw new CommandError.INVALID_ARG ("Too many search criterias.  Missing \"?");
        }

        var result = content_directory.search (this.container,
                                               args[1],
                                               filter,
                                               start,
                                               requested,
